...
// Game Initialization after player names are chosen
function gameInitialization(player1, player2) {
    ...
    const displayController = (() => {
        ...
        const cleanGameboard = () => {
            gameCells.forEach(cell => {
                cell.textContent = '';
                cell.classList.remove('occupied'); // Clear 'occupied' state
            });
        }
    ...
    const game = ((firstPlayer, secondPlayer) => {
        ...
        const makePlayerMove = (cell, player) => {
            if (cell.classList.contains('occupied')) return true; // Check if cell is occupied
